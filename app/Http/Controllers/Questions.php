...
class Questions extends Controller
{
    ...
    public function add_comments($id, Request $request)
    {
        ...
        if(Auth::user()->permission == true){
            $question_id = $id;
            DB::table("questions")->where('id', $question_id)->update(["status" => "answered"]);
    ...
    public function resolve($id, Request $request){
        // dd($id);
        if(Auth::user()->permission == false){
            $question_id = $id;
            DB::table("questions")->where('id', $question_id)->update(["status" => "resolved"]);
            // echo "<script> alert('Only clients can resolve ticket'); </script>";
            return back();
        } else {
            echo "<script> alert('Only clients can resolve ticket'); </script>";
            return back();
